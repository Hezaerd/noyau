...
import { ensurePierreIconSprite, resolvePierreIconForEntry } from "@/lib/pierre-icons"
...
type IconColorToken = keyof typeof ICON_COLORS

const isIconColorToken = (value: string): value is IconColorToken => value in ICON_COLORS

const colorForToken = (token: string | undefined, theme: "light" | "dark"): string => {
  const pair =
    token !== undefined && isIconColorToken(token) ? ICON_COLORS[token] : ICON_COLORS.default
  return pair[theme === "light" ? 0 : 1]
}
...
export const PierreEntryIcon = memo(function PierreEntryIcon(props: {
  ...
  return (
    <svg
      ...
      style={{ color: colorForToken(icon.token, theme) }}
      viewBox="0 0 16 16"
    >
      <use href={`#${icon.name}`} />
    </svg>
