...
const PIERRE_ICON_SPRITE_ID = "noyau-pierre-file-icon-sprite"

const NOYAU_FILE_ICON_SPRITE = `
<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" aria-hidden="true">
  <symbol id="noyau-file-icon-package-json" viewBox="0 0 32 32">
    <path d="M2 2H30V30H2" fill="#c12127" />
    <path d="M7.25 7.25h17.5v17.5h-3.5v-14H16v14H7.25" fill="#fff" />
  </symbol>
  <symbol id="noyau-file-icon-tsconfig" viewBox="0 0 32 32">
  ...
  <symbol id="noyau-file-icon-agents" viewBox="0 0 32 32">
  ...
  <symbol id="noyau-file-icon-claude" viewBox="0 0 32 32">
  ...
  <symbol id="noyau-file-icon-readme" viewBox="0 0 32 32">
  ...
  <symbol id="noyau-file-icon-pnpm" viewBox="0 0 32 32">
...
export const NOYAU_PIERRE_ICONS = {
  set: "complete",
  colored: true,
  spriteSheet: NOYAU_FILE_ICON_SPRITE,
  byFileName: {
    "package.json": "noyau-file-icon-package-json",
    "tsconfig.json": "noyau-file-icon-tsconfig",
    "agents.md": "noyau-file-icon-agents",
    "claude.md": "noyau-file-icon-claude",
    "readme.md": "noyau-file-icon-readme",
    "pnpm-lock.yaml": "noyau-file-icon-pnpm",
    "pnpm-workspace.yaml": "noyau-file-icon-pnpm",
  },
} satisfies FileTreeIcons

const completeIconResolver = createFileTreeIconResolver(NOYAU_PIERRE_ICONS)
...
type LanguageExtensionAlias = keyof typeof LANGUAGE_EXTENSION_ALIASES

const isLanguageExtensionAlias = (value: string): value is LanguageExtensionAlias =>
  value in LANGUAGE_EXTENSION_ALIASES

export const syntheticFileNameForLanguageId = (languageId: string): string => {
  const normalized = languageId.toLowerCase()
  if (isLanguageExtensionAlias(normalized)) {
    return `file.${LANGUAGE_EXTENSION_ALIASES[normalized]}`
  }
...
export const hasSpecificPierreIconForFileName = (fileName: string): boolean =>
  resolvePierreIconForEntry(fileName, "file")?.token !== "default"

export const ensurePierreIconSprite = (): void => {
  if (!("document" in globalThis) || document.getElementById(PIERRE_ICON_SPRITE_ID) !== null) {
    return
  }
  ...
  container.innerHTML = `${getBuiltInSpriteSheet("complete")}${NOYAU_FILE_ICON_SPRITE}`
