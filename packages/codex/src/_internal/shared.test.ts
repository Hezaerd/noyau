...
it.effect("preserves schema decode diagnostics without deriving the message from the cause", () =>
  Effect.gen(function* () {
    ...
    assert.equal(error.message, "Invalid payload for method 'thread/start' during 'decode-payload'")
...
it.effect("preserves schema encode diagnostics", () =>
  Effect.gen(function* () {
    ...
    assert.equal(error.message, "Invalid payload for method 'thread/start' during 'encode-payload'")
    assert.isTrue(Schema.isSchemaError(error.cause))
  }),
)
...
it.effect("keeps invalid payload values only in the exact schema cause", () =>
  Effect.gen(function* () {
    ...
    const cause = yield* decodeNestedNumberPayload({ profile: { token: secret } }).pipe(Effect.flip)
...
it.effect("retains the request-handler error as the internal error cause", () =>
  Effect.gen(function* () {
    ...
    assert.equal(error.message, "Codex App Server request handler failed for method 'thread/start'")
