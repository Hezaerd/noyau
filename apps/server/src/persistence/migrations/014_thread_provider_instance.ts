...
/**
 * SQLite ne droppe pas un CHECK : rebuild pour ouvrir `provider` à tout
 * instance id. La contrainte fermée omettait déjà `claude`.
 */
const migration = Effect.gen(function* () {
  ...
  yield* sql`
    INSERT INTO projection_threads_new (
      ...
      listed_at, context_used, context_window
    )
    SELECT
      thread_id, project_id, title, provider, runtime_mode, status,
      created_at, updated_at, archived_at, model_id, reasoning_effort,
