...
const defectMessage = (cause: unknown) =>
  cause instanceof Error ? cause.message : "ACP request failed"
...
export const callRpc = <A>(
  ...
  effect.pipe(
    ...
    Effect.catchDefect((cause) =>
      Effect.fail(
        AcpError.AcpRequestError.internalError(defectMessage(cause), undefined, {
          method,
...
const JsonRpcId = Schema.Union([Schema.Finite, Schema.String])
...
export const jsonRpcRequest = <A, I>(method: string, params: Schema.Codec<A, I>) =>
  Schema.Struct({
    ...
    params,
    headers: Schema.optionalKey(JsonRpcHeaders),
  })
