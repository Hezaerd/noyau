...
> **Statut : partiellement remplacé.** Le SQL explicite, le décodage `Schema` et le `Migrator`
> Effect restent en vigueur. Le store v0.1 est `node:sqlite` + `TxQueue`
> ([ADR-0012](0012-sqlite-locale-et-txqueue.md)), plus de PostgreSQL, PGlite ni outbox SQL.

Statut historique : accepté pour le SQL explicite — étendu à PGlite locale par l'ADR-0009.

La couche de persistance (event log append-only, receipts, outbox, leases) utilisait `@effect/sql-pg`
